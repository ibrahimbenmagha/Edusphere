...
import {setTokenAndRoleInLocalStorage,getUserFromLocalStorage, getTokenFromLocalStorage, getRoleFromLocalStorage, setCheckAdmin, getCheckAdmin
} from '../../../localStorageUtils'; 



import "./CreateSchoolUser.css";
...
export default function  CreateSchoolUser() {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosInstance.get('/getAllCitiesWithSectors');
  ...
  const handleResidenceChange = (value, selectedOptions) => {
   ...
   const onFinish = async (values) => {
    ...
    confirm({
      ...
      onOk: async () => {
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/CreateSchoolByUser', payload);
